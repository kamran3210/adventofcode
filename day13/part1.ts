...
function findMirrorPosition(grid: string[]) {
    ...
    for (const line of grid) {
        // find vertical reflection
        possibleMirrorPositions = possibleMirrorPositions.filter(x => getPossibleMirrorPositions(line, possibleMirrorPositions).has(x));
    }
    return possibleMirrorPositions.at(-1);
}

function getPossibleMirrorPositions(line: string, availablePositions: number[]) {
